...
VERSION = "V11.62 SIGNAL LOOSEN PASS"
...
WATCH_SCORE = 38
SIGNAL_SCORE = int(os.getenv("SIGNAL_SCORE", "45"))
SCAN_INTERVAL = 12
...
MIN_MOMENTUM_SIGNAL = int(os.getenv("MIN_MOMENTUM_SIGNAL", "1"))
MIN_MC_GROWTH = 1.000
...
LIQ_CONFIRM_MAX_DROP_PCT = float(os.getenv("LIQ_CONFIRM_MAX_DROP_PCT", "40.0"))
...
_PUMP_LP_BYPASS_COUNTER = {"count": 0}
...
def is_pump_migrated_market(report):
    """
    Pump.fun/PumpSwap migrations burn the LP tokens as a guaranteed part of
    the protocol's own migration mechanism (not something that varies per
    token the way a manually-added Raydium pool's lock does). RugCheck's
    markets[] entries for these pools don't always populate the generic
    lp-lock percentage fields our parser reads, which was showing up as a
    false "LP not locked" block on tokens that are actually safe by design.
    Detect PumpSwap/pump.fun-origin markets directly instead of relying on
    a percentage field that may not be populated for this pool type.
    """
    if not isinstance(report, dict):
        return False
    ...
    for market in report.get("markets") or []:
        ...
        blob = " ".join(str(market.get(k, "")) for k in (
            "marketType", "type", "programId", "program", "source", "dex", "label"
        )).lower()
        if "pump" in blob or PUMP_AMM_PROGRAM.lower() in blob:
            return True
    return False
...
def calculate_score(pair, report):
    ...
    # V11.61: Pump.fun/PumpSwap migrations guarantee LP burn by protocol
    # design — don't require a parsed percentage for these, treat as locked.
    if has_lp_market and lp_pct is None and is_pump_migrated_market(report):
        lp_pct = 100.0
        _PUMP_LP_BYPASS_COUNTER["count"] = _PUMP_LP_BYPASS_COUNTER.get("count", 0) + 1
...
def auto_scanner():
    ...
    "momentum_pass": 0, "liq_confirmed": 0, "liq_wait": 0, "liq_drop_block": 0, "clone_block": 0, "signal_gate_pass": 0,
        ...
        or (top10 is not None and top10 < HOLDER_TOP10_MAX)
                    ...
                    # V11.60: top1/LP-lock/min-age were previously only checked
                    # inside basic_signal_safe(), which the main pipeline never
                    # calls directly — it was only reached deep inside
                    # strong_signal() at the very last step. A candidate could
                    # sail through RUG_OK/AUTH_OK/SAFE/SCORE/TREND/MOMENTUM with
                    # these three never evaluated, then silently die at the
                    # final gate with no visibility. Folding them in here makes
                    # the funnel stats (and the funnel itself) match reality.
                    gate_ok = auth_ok
                    if gate_ok and result.get("top1") is not None and result["top1"] >= TOP1_MAX_PCT:
                        gate_ok = False
                        stats["top1_fail"] = stats.get("top1_fail", 0) + 1
                    if gate_ok and result.get("has_lp_market") and (
                        result.get("lp_locked_pct") is None or result["lp_locked_pct"] < LP_LOCK_MIN_PCT
                    ):
                        gate_ok = False
                        stats["lp_lock_fail"] = stats.get("lp_lock_fail", 0) + 1
                    if gate_ok and (
                        result.get("age_hours") is None
                        or result["age_hours"] < (MIN_PAIR_AGE_MINUTES / 60.0)
                    ):
                        gate_ok = False
                        stats["min_age_fail"] = stats.get("min_age_fail", 0) + 1
                    if gate_ok: stats["gate_ok"] = stats.get("gate_ok", 0) + 1
                    auth_ok = gate_ok
...
Yeni giris icin uygun degil."""
            ...
            if now_diag - last_diag_send >= 300 and stats.get("watch", 0) == 0 and stats.get("signal", 0) == 0:
                diag = (
                    ...
                    f"> GATE_OK={stats.get('gate_ok',0)} (top1_fail={stats.get('top1_fail',0)} lp_fail={stats.get('lp_lock_fail',0)} age_fail={stats.get('min_age_fail',0)} pump_lp_bypass={_PUMP_LP_BYPASS_COUNTER.get('count',0)}) "
