...
VERSION = "V11.56 FINAL GATE VISIBILITY"
...
Yeni giris icin uygun degil."""
            ...
            if now_diag - last_diag_send >= 300 and stats.get("watch", 0) == 0 and stats.get("signal", 0) == 0:
                diag = (
                    ...
                    f"FINAL GATE BREAKDOWN: SCORE_FAIL={stats.get('score_fail',0)} "
                    f"BUY_FAIL={stats.get('buy_fail',0)} "
                    f"VOLUME_FAIL={stats.get('volume_fail',0)} "
                    f"MC_FAIL={stats.get('mc_fail',0)} "
                    f"TREND_FAIL={stats.get('trend_fail',0)} "
                    f"MOMENTUM_FAIL={stats.get('momentum_fail',0)}\n"
