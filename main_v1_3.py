...
MIN_MC_GROWTH = 1.005
MAX_PAIR_AGE_HOURS = 2.0  # FIRST RUN ONLY: automatic entries limited to first 2 hours
    ...
    try:
        ...
        if ca:
            payload["reply_markup"] = json.dumps({
                "inline_keyboard": [[
                    {"text": "🚀 AXIOM'DA AÇ", "url": axiom_token_url(ca)}
                ]]
            }, ensure_ascii=False)
...
Major X: @{result.get("catalyst", {}).get("major_account") if result.get("catalyst", {}).get("major_account") else "YOK/DOĞRULANMADI"}
...
def crash_guard(result):
    # V1.3 FIRST-RUN HARD GATE: block old pump/dump and deep-crash rebounds from every entry lane.
    if not result:
        return False
    p1 = num(result.get("price1h"))
    p6 = num(result.get("price6h"))
    p24 = num(result.get("price24h"))
    ...
    if p1 is not None and p1 < MAX_SIGNAL_DROP_1H:
        return False
    if age is not None and age >= 1.0 and p6 is not None and p6 < MAX_CRASH_DROP_6H:
        return False
    if age is not None and age >= 6.0 and p24 is not None and p24 < MAX_CRASH_DROP_24H:
        return False
    return True
            ...
            for ca in candidates:
                try:
                    ...
                    if (
                        ...
                        # V1.1: HIKAYE GIR cannot be triggered by a token's own social link alone.
                        _cat = result.get("social_catalyst", {}) or {}
                        _cat_score = float(_cat.get("score") or 0)
                        _cat_news = int(_cat.get("news_count") or _cat.get("news") or 0)
                        _cat_reddit = int(_cat.get("reddit_count") or _cat.get("reddit") or 0)
                        _cat_major = bool(_cat.get("major_x_verified") or _cat.get("major_verified"))
                        verified_external_catalyst = _cat_score > 0 and (_cat_major or _cat_news > 0 or _cat_reddit > 0)
                        story_entry_ok = bool(story_signal and verified_external_catalyst and momentum > 0)

                        if story_entry_ok and not (fast_decision or trajectory_signal or volume_signal or ruru_signal):
                            karar_label = "HIKAYE GIR"
                        el# V1.1: GUCLU label needs positive momentum and no material liquidity slide.
                        try:
                            _prev_liq = float((old_metrics or {}).get("liq") or 0)
                        except Exception:
                            _prev_liq = 0.0
                        _curr_liq = float(result.get("liq") or 0)
                        liq_stable_for_guclu = True
                        if _prev_liq > 0 and _curr_liq > 0:
                            liq_stable_for_guclu = (((_curr_liq-_prev_liq)/_prev_liq)*100.0) >= -10.0
                        guclu_continuity_ok = liq_stable_for_guclu and momentum > 0

                        if fast_decision == "GUCLU GIR":
                            karar_label = "GUCLU GIR" if (price_allow_guclu and allow_quality_guclu and guclu_continuity_ok) else "GIR"
...
Major X: @{catalyst.get("major_account") if catalyst.get("major_account") else "YOK/DOĞRULANMADI"}
...
📊 AUTO Market: $3K–$12K
✅ Quality Gate: buy>=5, hacim>=$500, buy/sell>=1.10, late-pump<=+180%\n📡 /radar teşhisi: AKTİF\n🧩 Single Engine: AKTİF""")
        return
