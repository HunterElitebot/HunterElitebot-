...
VERSION = "V11.34 RURU MULTI SOURCE FIX"
...
def gecko_new_candidates(force=False):
    """Keyless GeckoTerminal Solana new-pools feed."""
    now = time.time()
    with source_feed_lock:
    ...
    for page in (1, 2, 3):
        ...
        try:
            payload = get_json(url, timeout=15, headers={"accept": "application/json"})
            included_map = {}
            for obj in (payload.get("included") or []) if isinstance(payload, dict) else []:
                ...
                addr = _valid_candidate_mint(attrs.get("address"))
                if oid and addr:
            ...
            for pool in (payload.get("data") or []) if isinstance(payload, dict) else []:
                ...
                for rel_name in ("base_token", "quote_token"):
                    ...
                    ca = included_map.get(rid)
                    if not ca and "_" in rid:
                        ca = _valid_candidate_mint(rid.split("_", 1)[-1])
                    if ca and ca not in seen:
                        seen.add(ca)
                        found.append(ca)
        except Exception as e:
            errors.append(f"p{page}:{type(e).__name__}")

    return _cache_source_result("GECKO", found, ";".join(errors[:4]))
...
def _raydium_row_mints(row):
    ...
    for key in ("mintA", "mintB"):
        ...
        if isinstance(obj, dict):
            ca = _valid_candidate_mint(obj.get("address") or obj.get("mint"))
        else:
            ca = _valid_candidate_mint(obj)
        if ca:
            out.append(ca)
    for key in ("mintAAddress", "mintBAddress", "baseMint", "quoteMint"):
...
def _meteora_row_mints(row):
    ...
    for key in (
        ...
        if ca:
            values.append(ca)
    for key in ("token_a", "token_b", "tokenA", "tokenB"):
        obj = row.get(key)
        if isinstance(obj, dict):
            ca = _valid_candidate_mint(obj.get("mint") or obj.get("address"))
            if ca:
                values.append(ca)
    return values
...
def meteora_new_candidates(force=False):
    ...
    for url in urls:
        try:
            ...
            # Preserve API order; freshness is still enforced later by pairCreatedAt.
            for row in rows[:250]:
...
def discovery_candidates():
    ...
    # If Birdeye is empty, other feeds may fill its unused slots up to RADAR_TARGET.
    if len(selected) < RADAR_TARGET:
        already = set(selected)
        for source in ("GECKO", "RAYDIUM", "METEORA", "DEX", "BIRDEYE"):
            for ca in buckets[source]:
                if ca in already:
                    continue
                selected.append(ca)
                already.add(ca)
