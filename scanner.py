    ...
    def get_json(self, url: str) -> Dict[str, Any]:
        ...
        for attempt in range(1, MAX_RETRY + 1):
            try:
                ...
                logger.warning(
                    "API denemesi başarısız attempt=%s/%s error=%s",
                    attempt,
                    MAX_RETRY,
                    exc,
                )
...
class DexScreenerClient:
    ...
    def get_pairs(self, contract: str) -> list:
        data = self.http.get_json(
            DEXSCREENER_URL.format(contract)
        )

        pairs = data.get("pairs") or []

        return pairs if isinstance(
            pairs,
            list
        ) else []
